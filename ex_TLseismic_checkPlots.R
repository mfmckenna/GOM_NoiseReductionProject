...
## Seismic ####
# G:\My Drive\ActiveProjects\COA\NFWF_GOM_workingDrive\data\Seismic\Seisimic TL
lsFiles = list.files( path = paste0(wrkDir, "data\\Seismic\\Seisimic TL\\"), pattern = ".shp", recursive=TRUE, full.names = T )

namTL = NULL
for (ii in 1: length( lsFiles )){
  namTL[ii] <- gsub(".shp", "", basename( lsFiles [ii]) )
  tmp = st_read(lsFiles[ii])
  tmp = tmp %>% st_set_crs(4326)
  tmp = st_as_sf(tmp)
  assign(namTL[ii],tmp)
...
TX_63_lLF = ggplot(data = world) +
  ...
  geom_sf(data = TX_12092020_0063Hz_0100m_SL204_7dB_NL081dB_100m, color = "green", alpha=.1) + 
  geom_sf(data = TX_12092020_0063Hz_0100m_SL212dB_NL081dB_100m, color = "orange",  alpha=.1) + 
  #set up
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  #ggtitle("Figure 1A. 63 Hz Transmission Loss Results (to 1000 km) \n
          # SNR: low-frequency hearing threshold 81 dB \n
           #Sources: orange = Air Gun SL 212 dB | green = Marine Vibrator SL 204.7 dB ") + 
  coord_sf(xlim = c(-100, -82), ylim = c(24, 31), expand = FALSE) 
TX_63_lLF

TX_125_lLF = ggplot(data = world) +
  geom_sf( ) +
  #geom_sf(data = df2, color = "blue",   size = .2, alpha = .5)  +
  geom_sf(data = region, alpha=0.1) + 
  geom_sf(data = dataBATH, alpha=0.2) + 
  #LargeVessel Listening Spaces- LF
  geom_sf(data = TX_12092020_0125Hz_0100m_SL205dB_NL075dB_100m, color = "orange", alpha=.1) + 
  geom_sf(data = TX_12092020_0125Hz_0100m_SL160dB_NL075dB_100m,  color = "green", alpha=.1) + 
  #set up
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  #ggtitle("Figure 1B. 63 Hz Transmission Loss Results (to 1000 km) \n
          # SNR: high-frequency hearing threshold 147 dB \n
           #Sources: orange = Air Gun SL 212 dB | green = Marine Vibrator SL 204.7 dB 
           #") + 
  coord_sf(xlim = c(-100, -82), ylim = c(24, 31), expand = FALSE) 
TX_125_lLF

NO_63_lLF = ggplot(data = world) +
  geom_sf( ) +
  #geom_sf(data = df2, color = "blue",   size = .2, alpha = .5)  +
  geom_sf(data = region, alpha=0.1) + 
  geom_sf(data = dataBATH, alpha=0.2) + 
  #LargeVessel Listening Spaces- LF
  geom_sf(data = NO_09042020_0063Hz_0100m_SL204_7dB_NL081dB_100m, color = "green", alpha=.1) + 
  geom_sf(data = NO_09042020_0063Hz_0100m_SL212dB_NL081dB_100m, color = "orange",  alpha=.1) + 
  #set up
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  #ggtitle("Figure 1A. 63 Hz Transmission Loss Results (to 1000 km) \n
...
NO_125_lLF = ggplot(data = world) +
  geom_sf( ) +
  #geom_sf(data = df2, color = "blue",   size = .2, alpha = .5)  +
  geom_sf(data = region, alpha=0.1) + 
  geom_sf(data = dataBATH, alpha=0.2) + 
  #LargeVessel Listening Spaces- LF
  geom_sf(data = NO_09042020_0125Hz_0100m_SL160dB_NL075dB_100m, color = "orange", alpha=.1) + 
  geom_sf(data = NO_09042020_0125Hz_0100m_SL205dB_NL075dB_100m, color = "green", alpha=.1) + 
  #set up
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  #ggtitle("Figure 1B. 63 Hz Transmission Loss Results (to 1000 km) \n
...
p3_63 = ggplot(data = world) +
  ...
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  ggtitle("Figure 1c. 63 Hz Transmission Loss Results (to 1000 km) \n
           Sources: orange = Air Gun SL 212 dB | green = Marine Vibrator SL 204.7 dB \n
           SNR: low-frequency hearing threshold 81 dB") + 
  coord_sf(xlim = c(-102.15, -80), ylim = c(22, 33), expand = FALSE)

p4_63 = ggplot(data = world) +
  geom_sf( ) +
  #geom_sf(data = df2, color = "blue",   size = .2, alpha = .5)  +
  geom_sf(data = region, alpha=0.1) + 
  geom_sf(data = dataBATH, alpha=0.2) + 
  #LargeVessel Listening Spaces- LF
  geom_sf(data = NO_09042020_0063Hz_0100m_SL204_7dB_NL147dB_100m, color = "green", alpha=.1) + 
  geom_sf(data = NO_09042020_0063Hz_0100m_SL212dB_NL147dB_100m, color = "orange", alpha=.1) + 
  #set up
  geom_sf(data = sites, color = "black", size = 1)  +
  geom_text(data  = dataPAM, aes(x=Longitude , y=Latitude +.3,  label = SiteID  ), size = 3)  +
  ggtitle("Figure 1c. 63 Hz Transmission Loss Results (to 1000 km) \n
  ...
  coord_sf(xlim = c(-102.15, -80), ylim = c(22, 33), expand = FALSE)
library(gridExtra)
grid.arrange(p1_63, p2_63, nrow = 1) # Texas
grid.arrange(p3_63,p4_63)
